...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  if (currentUser.changesPasswordAfter(decoded.iat)) {
    return new new AppError(
      ' User recently changed apssword! Please log in again.',
      401,
    )();
  }
...
exports.isLoggedIn = async (req, res, next) => {
  try {
    if (req.cookies.jwt) {
      ...
      // 3) Check if user changed password after token
...
exports.restrictTo = (...roles) => {
  return (req, res, next) => {
    ...
    if (!roles.includes(req.user.role)) {
      return next(
        new AppError('You do not habe permission to perform this action', 403),
      );
    }
